...
export default function EficienciaEnergeticaPage() {
  ...
  const consumosRef = useMemoFirebase(() => {
    ...
    return query(
      collection(firestore, 'empresas', profile.empresaId, 'consumosCombustible'),
      orderBy('fecha', 'desc')
    );
          ...
          <CardContent>
            <div className="space-y-4">
              {isLoading ? (
                ...
                <div className="text-center py-10 text-text-secondary italic border border-dashed border-border-dark rounded-xl">
                  No hay registros de combustible registrados para esta empresa.
                </div>
              ) : (
                consumos?.map(c => (
          ...
          <CardContent className="p-0 mt-4">
            <div className="h-64 bg-background-dark relative group">
              ...
              <div className="absolute bottom-4 left-4 right-4 space-y-2">
                <p className="text-xs text-text-secondary italic text-center p-4">Registra abastecimientos para ver las estaciones en el mapa.</p>
              </div>
            </div>
          </CardContent>
        </Card>
